...
if __name__ == "__main__":
    ...
    # parser.add_argument('pretrained_model_path',                                                        #pretrained_model.path 추가함
    ...
    # checkpoint = torch.load(args.pretrained_model_path)                                       #pretrained weight 추가 
    model = ResNet1d(input_dim=(N_LEADS, args.seq_length),
                     blocks_dim=list(zip(args.net_filter_size, args.net_seq_lengh)),
                     n_classes=N_CLASSES,
                     kernel_size=args.kernel_size,
                     dropout_rate=args.dropout_rate)
    # model.load_state_dict(checkpoint['model'])                                                 #pretrained weight 추가 
