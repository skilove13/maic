...
dataset_path = 'E:/smhong/micai_ai_challange/dataset/ECG_adult_numpy_valid'

#ecg_adult_data_path를 glob.glob() 함수를 사용하여 리스트로 저장하는데 이름순으로 순차적으로 정렬
ecg_adult_data_path = glob.glob("E:/smhong/micai_ai_challange/dataset/ECG_adult_numpy_valid/ecg_adult_*.npy")
...
for i in range(len(npy_files)):
    file_path = "E:/smhong/micai_ai_challange/dataset/ECG_adult_numpy_valid/" + str(npy_files[i])
    #file_path = dataset_path + "/" + str(npy_files[i])
...
# # 리스트를 Numpy 배열로 변환
ecg_adult_data_valid = np.array(ecg_data_list)

# ecg_data_array의 shape 확인
print("ecg_adult_data_valid shape:", ecg_adult_data_valid.shape)

start = time.time()  # 시작 시간 저장
# npz 파일로 저장
np.savez('ecg_adult_data_valid.npz', ecg_adult_data_valid=ecg_adult_data_valid)
