...
# train_datasets = ['ecg_adult_data1.npz', 'ecg_adult_data0.npz', 'ecg_adult_data5.npz',
#                   'ecg_adult_data4.npz', 'ecg_adult_data3.npz', 'ecg_adult_data2.npz','ecg_adult_data6.npz']  # , 'ecg_child_data1.npz']
# y_train_datasets = ['y_adult_train1.npz', 'y_adult_train0.npz', 'y_adult_train5.npz',
#                     'y_adult_train4.npz', 'y_adult_train3.npz', 'y_adult_train2.npz','y_adult_train6.npz']  # ,'y_child_train1.npz']
# train_datasets = ['ecg_adult_data1.npz']
# y_train_datasets = ['y_adult_train1.npz']
...
#         if len(y_train_comb) == 3:
#             y_train_datasets_grouped.append(list(y_train_comb))

# train_datasets_grouped
train_datasets_grouped.append(list(train_datasets[0:4]))
y_train_datasets_grouped.append(list(y_train_datasets[0:4]))
# train_datasets_grouped.append(list(train_datasets[4:]))
# y_train_datasets_grouped.append(list(y_train_datasets[4:]))

# train_datasets_grouped.append(list(train_datasets[2:3]))
# y_train_datasets_grouped.append(list(y_train_datasets[2:3]))
# train_datasets_grouped.append(list(train_datasets[3:4]))
# y_train_datasets_grouped.append(list(y_train_datasets[3:4]))
# train_datasets_grouped.append(list(train_datasets[4:5]))
# y_train_datasets_grouped.append(list(y_train_datasets[4:5]))
# train_datasets_grouped.append(list(train_datasets[5:6]))
# y_train_datasets_grouped.append(list(y_train_datasets[5:6]))
# train_datasets_grouped.append(list(train_datasets[6:7]))
# y_train_datasets_grouped.append(list(y_train_datasets[6:7]))
#
# train_datasets_grouped.append(list(train_datasets[0:1]))
# y_train_datasets_grouped.append(list(y_train_datasets[0:1]))
# train_datasets_grouped.append(list(train_datasets[1:2]))
# y_train_datasets_grouped.append(list(y_train_datasets[1:2]))
# train_datasets_grouped.append(list(train_datasets[2:3]))
# y_train_datasets_grouped.append(list(y_train_datasets[2:3]))
# train_datasets_grouped.append(list(train_datasets[3:4]))
# y_train_datasets_grouped.append(list(y_train_datasets[3:4]))
...
# train_datasets_grouped 리스트 내부의 항목을 역순으로 배치
# train_datasets_grouped_reversed = [group[::-1]for group in train_datasets_grouped]
# y_train_datasets_grouped_reversed = [group[::-1]for group in y_train_datasets_grouped]
#
# train_datasets_grouped = train_datasets_grouped_reversed
# y_train_datasets_grouped = y_train_datasets_grouped_reversed
# train_datasets_grouped = train_datasets_grouped[0:]
# y_train_datasets_grouped = y_train_datasets_grouped[0:]
...
for i, (train_group, y_train_group) in enumerate(zip(train_datasets_grouped, y_train_datasets_grouped)):
    ...
    BATCH_SIZE = 64 #256 , single dataset
    ...
    with strategy.scope():
        #model = resnet19_model(dropout_rate=0.2)
        model = tf.keras.models.load_model(model_path + '/resnet19_dropout' + str(model_number) + '.h5')
    ...
    history = model.fit(train_dset, epochs=40, validation_data=valid_dset) #, callbacks=[early_stopping_callback])
    #
    ...
    model_number += 1
    # 모델 저장
    model.save(model_path + '/resnet19_dropout' + str(model_number) + '.h5')
    end_time = time.time()
