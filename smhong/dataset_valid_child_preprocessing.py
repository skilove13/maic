"""
1. ECG_adult_numpy_valid폴더의 파일들을 읽어서 리스트 형태로 저장
...
dataset_path = 'E:/smhong/micai_ai_challange/dataset/ECG_child_numpy_valid'

#ecg_adult_data_path를 glob.glob() 함수를 사용하여 리스트로 저장하는데 이름순으로 순차적으로 정렬
ecg_child_data_path = glob.glob("E:/smhong/micai_ai_challange/dataset/ECG_child_numpy_valid/ecg_child_*.npy")

print(len(ecg_child_data_path))
...
for i in range(len(npy_files)):
    file_path = "E:/smhong/micai_ai_challange/dataset/ECG_child_numpy_valid/" + str(npy_files[i])
    ecg_child_data.append(file_path)

# print(ecg_adult_data[0:30])
print("len(ecg_adult_data) = ",len(ecg_child_data))
...
# # 리스트를 Numpy 배열로 변환
ecg_child_data_valid = np.array(ecg_data_list)

# ecg_data_array의 shape 확인
print("ecg_adult_data_valid shape:", ecg_child_data_valid.shape)

start = time.time()  # 시작 시간 저장
# npz 파일로 저장
np.savez('ecg_child_data_valid.npz', ecg_child_data_valid=ecg_child_data_valid)
